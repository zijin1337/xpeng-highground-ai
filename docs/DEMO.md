...
证据 JSON 是本地 FastAPI/SQLite 运行记录，包含运行环境、各步骤请求路径、延迟、期望值、响应、事件/消息/授权/命令 ID 关联和断言结果。渲染前会校验事件链与 `input_sha256` 在遥测、事件查询和最新决策之间一致。明文单次授权令牌不会写入文件，只保留 SHA-256；API Key 也不会写入报告。报告明确记录：
